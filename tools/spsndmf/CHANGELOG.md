...
- Fixed a build error when MA Replace Object replaces an SPS plug or its mesh: the build now skips converting that plug's external animations (with a console warning) instead of failing.
  ...
  MA Replace Object で SPS Plug やそのメッシュを置き換えているとビルドがエラーで止まる問題を修正しました。該当 Plug の外部アニメーション変換はスキップされ（コンソールに警告が出ます）、ビルドは継続します。
- Fixed on/off style animations targeting the plug or its mesh (animations of properties without in-between values, such as visibility toggles) turning into smoothly interpolated curves after the build. They used to lose their stepped switching and could take unintended in-between states between keys. Plugs on static meshes are covered too.

  Plug やそのメッシュに対する ON / OFF 切り替えのアニメーション（表示の切り替えなど、中間値を持たないプロパティのアニメーション）が、ビルド後になめらかな補間に変わってしまう問題を修正しました。これまでは切り替わりが階段状ではなくなり、キーの間で意図しない中間状態を取ることがありました。静止メッシュの Plug も対象です。
- Fixed one of the curves disappearing entirely from the built avatar when a clip generated by another tool stacks two kinds of curves on the same property. The affected curve is now kept, but the stacked one is not converted for SPS (if it was a curve that drives SPS features, that effect will not work).

  他のツールが生成した特殊なアニメーション（同じプロパティに2種類のカーブが重なっているもの）があると、片方のカーブがビルド後に丸ごと消えていた問題を修正しました。該当するカーブは消えずに残りますが、重なっている側のカーブには SPS の変換が適用されません（SPS 関連の操作をするカーブだった場合、その効果は働きません）。

### Changed / 変更
- Updated the bundled VRCFury source to upstream 1.1426.0. Notable SPS changes:
  - SPS material patching was reworked to support more shaders (for example shaders whose vertex programs SPS previously could not handle), and reports clearer error messages when patching fails.
  - Post-Bake Actions on SPS Plugs now also run on Android (Quest) builds.
  - SPS sockets using legacy lights now pass their light info to SPS correctly (upstream fix).
  - The Green Screen Fix component now uses a more standard grabpass restoration, which may fix it on Linux editors (upstream change).
  - The legacy (light-based) socket type list gained **One-Way Ring**, matching the normal socket types.

  同梱の VRCFury ソースを上流 1.1426.0 に更新しました。SPS 関連の主な変更：
  - SPS のマテリアルパッチ処理が改修され、これまでパッチできなかったシェーダー（頂点プログラムの形が特殊なものなど）にも対応しました。パッチに失敗した場合のエラーメッセージも分かりやすくなりました。
  - SPS Plug の Post-Bake Actions が Android（Quest）ビルドでも実行されるようになりました。
  - レガシーライトを使う SPS Socket のライト情報が SPS に正しく渡るようになりました（上流修正）。
  - Green Screen Fix コンポーネントがより標準的な grabpass 復元方式になり、Linux エディタでの動作が改善する可能性があります（上流の変更）。
  - レガシー（ライト式）ソケットのタイプに、通常タイプと同じ **One-Way Ring** が追加されました。
