...
- **SPS2 は、触覚通知（OGB）の Receiver を「自分のクライアントで、かつ OSC 触覚アプリを起動している間」だけ有効にするゲートを持ちます**（SPS1 には無い仕組み）。このため、通常のプレイ（OSC アプリ未起動）では OGB の Receiver が無効になり、SPS2 が同時に有効化する Contacts は SPS1 より大幅に少なくなります。
