...
SPS Options は、アバター全体の SPS の動作をまとめて調整するためのオプションのコンポーネントです。付けなくても SPS は動作します（すべて既定値になります）。また、SPS Options 単体では何も行いません（[Plug](/spsndmf/plug/) または [Socket](/spsndmf/socket/) がアバターにあるときに効果を持ちます）。
