...
SPS2 は Socket の位置伝達に点光源を使わないため、このページで説明したような「同時に有効化できる Socket の数の実質的な上限」や「VRChat のライト関連設定への依存」がありません。詳しくは [SPS2 のしくみ](/spsndmf/how-it-works-sps2/) を参照してください。
